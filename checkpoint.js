...
function stringMasLarga(str) {
  ...
  arrayDeCadenas.forEach(element => {
    if(element.length > elMasLargo.length){
      elMasLargo = element;
    }
  });
  return elMasLargo;
...
function estaOffline(usuarios, nombre) {
  ...
  usuarios.forEach(element => {
    if(element.nombre === nombre && element.online){
      estado = !element.online
    }
  });
  return estado;
}
...
function actividadesEnComun(persona1, persona2) {
  ...
  // Tu código aca:
  var enComun = [];
  persona1.forEach(element1 => {
    persona2.forEach(element2 => {
      if(element1 === element2){
        enComun.push(element2);
      }
    });
  });
  return enComun;
...
function sumarElTipo(arreglo) {
  ...
  // Tu código aca:
  var arregloObjeto = [[arreglo[0],1]];
  var salir = true;
  var finDeCiclo = true;
  arreglo.reduce(function (acc, elemento){
    finDeCiclo = true;
    arregloObjeto.forEach(function(element,indice){
      if(salir && elemento == element[0] && finDeCiclo){
        element[1]++;
        finDeCiclo = false;
      }else if(elemento != element[0] && finDeCiclo && arregloObjeto.length-1 == indice){
        salir=false;
        finDeCiclo=true;
      }
      })
    if(!salir){
      arregloObjeto.push([elemento,1]);
      finDeCiclo = false;
      salir = true;
    }
  })
  return Object.fromEntries(arregloObjeto);
}
